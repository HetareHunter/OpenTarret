...
{
    [SerializeField] CustomRenderTexture _customRenderTexture;
    [SerializeField, Range(0.001f, 0.05f)] float _rippleSize = 0.01f;
    [SerializeField] int iterationPerFrame = 5;

    CustomRenderTextureUpdateZone _defaultZone;
    ...
    void Start()
    {
        ...
        _customRenderTexture.Initialize(); //初期化
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (LocalPointToUV(hitLocalPos, out var uv))
        {
            ...
            var interactiveZone = new CustomRenderTextureUpdateZone
            {
                ...
                updateZoneSize = new Vector2(_rippleSize, _rippleSize)
            };

            _customRenderTexture.SetUpdateZones
                (new CustomRenderTextureUpdateZone[] { _defaultZone, interactiveZone });
        }
    }
    ...
    bool LocalPointToUV(Vector3 localPoint, out Vector2 uv)
    {
        int index0;
        int index1;
        int index2;
        ...
        for (int i = 0; i < _meshTriangles.Length; i += 3)
        {
            ...
            //同一平面上に任意の座標が定義されているかどうかをチェック
            if (!CheckInPlane(localPoint, t1, t2, t3)) continue;

            //三角形の内部に存在するかどうか、境界面(辺の上、頂点の真上)もチェック
            if (!CheckOnTriangleEdge(localPoint, t1, t2, t3)
                && !CheckInTriangle(localPoint, t1, t2, t3)) continue;
    ...
    bool CheckInPlane(Vector3 p, Vector3 t1, Vector3 t2, Vector3 t3)
    {
        //各点同士の成すベクトルを計算
        var v1 = t2 - t1;
        var v2 = t3 - t1;
    ...
    bool CheckInTriangle(Vector3 p, Vector3 t1, Vector3 t2, Vector3 t3)
    {
    ...
    bool CheckOnTriangleEdge(Vector3 p, Vector3 t1, Vector3 t2, Vector3 t3)
    {
    ...
    /// <returns>頂点対象の座標が境界上にあればTrue</returns>
    bool CheckOnEdge(Vector3 p, Vector3 v1, Vector3 v2)
    {
    ...
    /// <returns></returns>
    Vector2 CalculateUV(Vector3 p, Vector3 t1, Vector2 t1UV, Vector3 t2, Vector2 t2UV, Vector3 t3, Vector2 t3UV)
    {
        //メインカメラ取得
        var renderCamera = Camera.main;
        //プロジェクション変換一歩手前のMVP行列 = プロジェクション行列 * ビュー(カメラ)行列 * モデル行列(ワールド座標←ローカル座標)
        ...
        //Shaderでは自動でラスタライズステージで行っているが、今回は自分でやる　透視変換というらしいです
        //Z(深度)は破棄
